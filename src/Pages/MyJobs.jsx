...
const MyJobs = () => {
  ...
  useEffect(() => {
    setIsLoading(true)
    fetch("http://localhost:3000/myJobs/franklindutch1@gmail.com")
      .then((res) => res.json())
      .then((data) => {
        setJobs(data);
        setIsLoading(false)
  ...
  console.log({jobs}, 'wetin be that')
              ...
              <table className="items-center bg-transparent w-full border-collapse ">
                ...
                {isLoading ? (<div className='flex items-center justify-center h-20'><p>Loading...</p></div>) : ( <tbody>
                  ...
                  <tr key={index}>
                    ...
                    <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4">
                    <button onClick={() => handleDelete(job._id)} className='bg-red-700 py-2 px-6 text-white rounded-sm'><Link to={`/edit-job/${job?._id}`}>Delete</Link></button>
                    </td>
                  </tr>
                    ))
                  }
